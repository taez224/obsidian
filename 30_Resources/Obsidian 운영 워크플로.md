...
공개 페이지의 `개발 노트`는 `개념·설계 / 문제 해결 / 도구·워크플로`로 나누고, 기술 태그로 주제를 찾는다. 공개 대상은 `20_Projects/obsidian-garden/config.json`의 각 개발 폴더 `files` 목록에서 선택한다. 새 노트를 만들거나 수정한 것만으로 공개 대상으로 추가하지 않는다. 일반적인 기술 설명, 문제 해결 과정, 필요한 클래스·메서드명은 유지할 수 있다. 외부 독자가 모르는 사내 제품명·업무 용어는 해당 문맥의 역할로 풀어 쓰며, 그 과정에서 기술적 의미를 바꾸지 않는다. 실제 접속 정보·자격증명·고객 정보는 본문과 첨부에 넣지 않는다. 사실 설명은 공식 자료와 확인된 기록으로 대조한다.
