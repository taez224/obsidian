...
공개 페이지의 `개발 노트`는 `개념·설계 / 문제 해결 / 도구·워크플로`로 나누고, 기술 태그로 주제를 찾는다. 세 개발 폴더의 일반 Markdown은 다음 사이트 빌드에 자동으로 포함하고, 이름이 `_` 또는 `.`으로 시작하는 파일·폴더, `DevLog`는 제외한다. `exclude`가 있으면 명시적 제외를 우선하며, 첨부파일은 기존의 별도 검토 목록을 따른다. 일반적인 기술 설명, 문제 해결 과정, 필요한 클래스·메서드명은 유지할 수 있다. 외부 독자가 모르는 사내 제품명·업무 용어는 해당 문맥의 역할로 풀어 쓰며, 그 과정에서 기술적 의미를 바꾸지 않는다. 실제 접속 정보·자격증명·고객 정보는 본문과 첨부에 넣지 않는다. 사실 설명은 공식 자료와 확인된 기록으로 대조한다.

개발 노트의 연결도 본문 wikilink를 기본으로 한다. 특정 개념을 설명하는 자리에서는 그 개념 노트를 바로 연결하고, 문서 전체가 이어지는 관계는 마지막 `## 연관된 노트`에 이유를 한 줄로 적는다. 개념의 구현, 구현의 적용 한계, 서로 구분해야 할 실패 원인처럼 실제 관계가 있을 때만 연결한다. 예를 들어 멀티테넌시 개념 노트와 향후 Spring·JPA 구현 노트는 원리와 적용의 관계로 연결할 수 있다. 아직 작성하지 않은 노트의 링크나 동일한 목록을 담는 `related` 메타데이터는 만들지 않는다. 공개된 노트 사이의 본문 링크는 사이트에서 연결·역링크로도 표시되므로 양방향 목록을 기계적으로 반복하지 않는다. 외부 자료 링크도 본문에서 출처를 부르는 어구에 거는 것을 기본으로 하고, 본문에 넣지 못한 자료만 `## 참고 자료`에 둔다.

Git 추적과 웹사이트 공개는 별개다. 사이트 공개 대상이 아닌 노트도 Git으로 올리면 저장소에서 보일 수 있다. 회사 내부 자료는 Git 제외 경로에 남기고, `DevLog`와 `_workspace`는 사이트에서도 제외한다. 자동 공개 폴더에는 공개 가능한 내용만 둔다.
...
병합·분리도 함께 검토할 때는 노트의 경계를 먼저 정하고 연결을 다듬는다. 구체적인 판단과 적용 절차는 `review-zettelkasten`을 따른다.
...
태그가 같거나 노트가 3개 이상이라는 이유만으로 만들지 않는다. 현재 주제 MOC는 [[AI 활용]]과 [[나를 이해하기]]이다. 다른 군집은 실제 탐색 수요가 생길 때까지 기존 링크와 Backlinks를 사용한다.
...
문장·헤딩·종결체의 공통 기준은 `.agents/guides/writing.md`를 따른다. 작업별 구성과 깊이는 해당 스킬에서 정한다.
...
- [[생각 캡쳐와 영구 노트 승격은 분리해야 한다]] - 빠른 기록과 의미 판단을 나누는 운영 경계
