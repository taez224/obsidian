---
summary: 배포 환경 DB에 더 높은 버전이 이미 적용되어 새 스크립트가 지나간 버전으로 판정된 것이며, 그 스크립트가 어느 환경에 적용됐는지에 따라 버전 조정과 보정 마이그레이션 중에서 고른다.
created: 2026-07-15
...
Flyway의 버전 판정과 outOfOrder 동작은 위 공식 문서로 확인했다. 실제 사례에서는 스크립트가 공유 환경에 적용되기 전이라 버전 번호를 뒤로 조정해 해결했고, outOfOrder는 쓰지 않았다.
