...
회사에서 문서 관리 시스템 구축 프로젝트 중, 한 Spring 기반 서비스에서(이하 Spring) PDF 텍스트 추출, 하이라이트, 분할 세 기능을 PDF 처리 전담 외부 서비스(이하 FastAPI)를 호출했는데, Spring 쪽에서 응답을 받는 형태는 셋 다 달랐다. 겉보기에는 일관성이 없어 보이지만, 각 기능에서 응답을 받은 다음 단계가 무엇을 요구했는지에 따라 형태가 정해졌다.

```mermaid
flowchart LR
    API["FastAPI 응답"]
    API -->|텍스트 추출| N1["값 하나만 필요"]
    N1 --> R1["문자열로 받아 동기 코드에 연결"]
    API -->|하이라이트| N2["내용을 볼 필요 없음"]
    N2 --> R2["버퍼 흐름을 S3 PUT에 그대로 연결"]
    API -->|분할| N3["ZIP을 풀어야 함"]
    N3 --> R3["임시 파일로 내린 뒤 엔트리별 업로드"]
```
