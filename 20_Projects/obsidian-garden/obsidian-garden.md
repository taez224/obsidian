...
- 홈: 라이브 그래프 히어로 + About 한 문장, 대표 글 1편, 글·생각 노트·개발 노트의 최근 기록을 종류마다 1편씩 표시.
- 노트 지도: 그래프 영역 크기에 맞춰 배치하고, 제목은 확대와 무관하게 13px로 표시한다. 노드 주변 8개 후보를 순서대로 시도해 겹치지 않는 자리를 우선하며, 호버하면 선택 전에 이웃 제목을 미리 보여준다. 범례는 주제·허브 필터를 제공한다. 데스크톱 패널·모바일 바텀시트.
- 리더: 메타 줄(종류 링크 · 날짜 · 읽는 시간 · 주제 · 발행처) → 제목 → 본문 → 이전·다음 편 링크와 목록 링크. 사이드바에 목차 레일, 연재 목록, 로컬 그래프, 참조·역참조.
- 공유·유입: 노트마다 OG 카드(빌드 생성), RSS 통합 피드와 종류별 세 피드, Umami Cloud 방문 통계.
...
- 개념·도구 정리: [[그래프의 노드 제목이 겹치지 않게 놓는 방법]], [[정적 사이트의 OG 카드와 방문 통계]].
- 지도 제목 실측(2026-09-06, 그래프 영역 830×598px·노드 33·간선 87): 두 줄 제목이 차지하는 영역의 합이 그래프 영역의 58%, 밀어내기 후처리 뒤에도 겹침 28쌍, 캔버스 2배(약 1660×1200)에서 0. 제목 13px 고정·노출 문턱 1.2배 기준으로 맞춤에서 허브 3개, 1.25배 29개, 1.95배 33개 전부 노출. 이웃 15개 허브는 맞춤 11개, 1.4배 14개. 첫 화면 전체 노출은 22개가 나와 빽빽해 되돌림 → 보류 후보 '2배 캔버스'의 근거.
