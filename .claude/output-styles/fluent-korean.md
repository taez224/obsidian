...
## 이 vault에서 추가로 지키는 사항

- 한국어로 출력되는 모든 결과물에 이 지침들을 적용합니다. 사용자에게 보고하는 문장뿐 아니라 노트와 글에도 적용합니다.

- 한국어 사전에 있는 어휘이며 뜻이 명확하더라도, 사용 빈도가 낮아서 통용되지 않는 어휘를 사용하면 소통의 효율성이 오히려 낮아지니 자제합니다. 대신 의미가 명확하며 실제로 통용되는 어휘를 우선적으로 선택합니다.

- 위의 지침들은 문장 내부를 규율합니다. 분량과 구조, 종결체, 형식은 `AGENTS.md`의 「글쓰기」 절과 각 스킬이 정하며, 어긋나면 그쪽을 따릅니다.

- 「글쓰기」 절의 "문장은 짧게"는 문장 수와 군더더기를 줄이라는 뜻이지, 문장에서 조사와 어미를 덜어내라는 뜻이 아닙니다. 두 규칙을 동시에 지킵니다.

- 헤딩을 짧은 명사구로 쓰는 규칙은 위 「문장 단위」 2번의 헤더·목록 예외에 해당하므로 그대로 지킵니다.

- 이 지침 본문이 "~합니다"체로 서술되어 있더라도, 산출물의 종결체는 그 산출물의 규칙을 따릅니다. 개발 노트는 평어체로 씁니다. 블로그에서는 종결어미 리듬을 기계적으로 균일하게 맞추지 않습니다.

<!-- 지침 원문 출처: https://github.com/snflkd/fluent-korean (MIT), 커밋 ce8683f0, 2026-08-23 기준.
     원문은 위의 「추가로 지키는 사항」 절 앞까지이며 한 글자도 고치지 않았다.
     Copyright (c) 2026 snflkd. MIT 전문은 같은 폴더의 LICENSE-fluent-korean에 있다. -->
