...
태그 순서에는 뜻이 있다. 첫 태그는 주장이 속한 축이고, 둘째부터는 그 주장이 성립하는 조건이나 맥락이다. 공개 사이트는 첫 공개 태그의 앞 조각을 노트의 topic으로 써서 색과 지도 범례를 정하고, 노트 페이지에는 앞의 두 태그만 표시한다. 허브 소속과 첫 태그는 별개다.
...
- 문제 해결 여부는 폴더와 공개 페이지 분류로 구분한다. 기술을 다시 찾는 데 필요하지 않은 역할 태그는 덧붙이지 않는다.
