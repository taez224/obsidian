...
## 백업 슬라이드: 실제 프롬프트 & 응답 예시

### 프롬프트
```
TenantConfig를 record로 바꿔줘.
영향 파일과 단계별 계획을 알려줘.
테스트는 ./gradlew test로 검증할 거야.
```
...
1. **영향 파일 목록**
   - TenantConfig.java
   - TenantConfigFactory.java
   - 관련 테스트 파일들

2. **단계별 계획**
   - 모델 변환 → 팩토리 수정 → 테스트 업데이트
