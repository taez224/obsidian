...
오히려 하네스는 얇을수록 좋다. 공통 실행 환경, 테스트, 기록, 승인 경계 정도만 가볍게 잡아야 각자의 작업 방식이 살아난다. 너무 두꺼운 프로세스는 AI 활용을 표준화하는 것처럼 보이지만, 실제로는 잘 쓰는 사람의 감각까지 눌러버릴 수 있다.
...
팀이 두껍게 쌓아야 하는 것은 절차가 아니라 작업법이다. 좋은 리뷰 질문, 테스트를 뽑는 방식, 장애 원인을 좁혀 가는 순서, 도메인 맥락을 모델에게 설명하는 요령 같은 것들이다. 누군가 좋은 방식으로 AI를 써서 문제를 풀었다면 그것이 개인의 요령으로 끝나지 않게 해야 한다. 반복 가능한 형태로 남을 때 비로소 팀의 스킬이 된다.
...
run_id: 2026-06-01-002
mode: fast
genre: blog
rewritten_length: 6028
...
category_counts_after:
  - overt_thin_fat_phrase: 0
  - naturalized_harness_skill_concept: 2 paragraphs
  - targeted_slop_patterns: none found by rg
self_check: 6/6
quality_grade: A
highlights:
  - Added the thin harness, fat skills idea without naming it in English.
  - Framed a thin harness as lightweight shared execution, test, record, and approval boundaries.
  - Framed fat skills as reusable team work methods such as review questions, testing patterns, debugging flow, and domain-context prompting.
  - Preserved the company technical blog tone and the team-level AI capability thesis.
residual_findings:
  - The phrase team skill remains intentionally because it fits the article's team-capability framing.
-->
