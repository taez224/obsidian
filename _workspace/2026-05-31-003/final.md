---
title: "AI 시대의 개발팀은 생성량보다 회수율을 봐야 한다"
created: 2026-05-31
tags:
  - blog
  - AI
...
status: draft
author: TaeZ
summary: "AI 코딩을 팀의 생산성으로 만들려면 토큰 사용량이나 생성 코드량보다 회수율을 봐야 한다. 설명, 검증, 학습, 이력 관리로 산출물을 팀의 지식으로 전환하는 운영 하네스가 필요하다."
---

# AI 시대의 개발팀은 생성량보다 회수율을 봐야 한다

AI 코딩 도구를 도입하면 처음에는 숫자가 잘 보인다. 프롬프트 수, 토큰 사용량, 에이전트 실행 횟수, 생성된 코드량, 자동화된 PR 수. 모두 측정하기 쉽고 보고하기도 좋다.

문제는 그 숫자들이 팀에 무엇이 남았는지를 잘 말해 주지 않는다는 데 있다.

토큰을 많이 썼다고 더 깊이 이해한 것은 아니다. 더 많은 에이전트를 돌렸다고 더 나은 시스템이 남는 것도 아니다. 코드가 많이 생성됐다고 팀의 지식이 그만큼 쌓이는 것도 아니다.

오래전에 Dijkstra는 코드 라인을 “생산한 양”이 아니라 “소비한 비용”으로 봐야 한다고 했다. 코드 줄 수가 생산성을 설명하지 못했듯, AI 시대의 토큰 사용량이나 생성 코드량도 팀의 성숙도를 설명해 주지는 못한다.

AI 도입 이후 팀이 물어야 할 질문은 조금 다르다.

무엇이 생성됐는가가 아니라, 무엇이 회수됐는가.

## 산출물은 자동으로 지식이 되지 않는다

AI가 만든 코드는 저장소에 들어간다. 하지만 저장소에 들어갔다고 팀의 지식이 되지는 않는다.

외부에서 들어온 지식은 언제나 변환 과정을 거친다. 예제 코드는 우리 버전과 맞아야 하고, 테스트는 우리 실패 조건을 설명해야 하고, 설계 판단은 우리 시스템의 경계와 맞아야 한다. 이 과정을 거치지 않으면 산출물은 남아도 팀의 이해는 남지 않는다.
...
AI 사용량 자체가 문제는 아니다. 차이를 만드는 것은 인지 참여 방식이다. AI를 외주 계약자처럼 쓰면 결과는 빨리 나오지만 학습은 남지 않을 수 있다. AI를 동료 리뷰어처럼 쓰면 반례를 묻고 실패 조건을 캐묻고 대안을 비교하게 하면서 사고를 강화할 수 있다.
...
PR 템플릿, 테스트, 실행 로그, 리뷰 규칙은 AI를 느리게 만들기 위한 장치가 아니다. AI가 만든 산출물을 팀의 지식 체계 안으로 회수하기 위한 장치다.
...
프롬프트나 컨텍스트를 그대로 남기자는 뜻은 아니다. 민감한 정보가 섞일 수 있기 때문이다. 더 중요한 것은 “AI가 썼다”는 면책 표시가 아니라, 나중에 문제가 생겼을 때 그 판단이 어디서 왔는지 따라갈 수 있는 구조다.
...
AI 시대의 개발팀은 생성량보다 회수율을 봐야 한다. 산출물이 코드로만 남으면 비용은 사람에게 쌓인다. 산출물이 설명, 검증, 학습, 이력으로 회수될 때 비로소 팀의 생산성이 된다.
...
run_id: 2026-05-31-003
mode: fast
genre: blog
original_length: 4395
rewritten_length: 4395
...
high_signal_metrics:
  ...
  - ending_comma_rate: 0.16666666666666666
category_counts_before:
  - macro_overload: 1 integrated article
  - report_like_ordering: medium
  - ordinal_loop_style: 3 spans
category_counts_after:
  - macro_overload: reduced to team operations scope
  - report_like_ordering: reduced
  - ordinal_loop_style: 0 targeted spans
self_check: 6/6
quality_grade: A
highlights:
  - Kept recovery rate and harness as the main technical-blog argument.
  - Moved tariff/debt metaphor out of the foreground.
  - Rewrote the three loops without numbered report phrasing.
  - Kept Git provenance as a practical future-facing section.
residual_findings:
  - Reference list remains because this version is intended for a technical blog.
-->
